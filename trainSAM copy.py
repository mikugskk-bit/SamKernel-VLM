...
from tqdm import *
import torchmetrics
from MydataLoader import MyDatasetK,PETCTDataset
import argparse
...
def run():
    models = ['EM-U','EM-SAM','EM-SAMF','EM-SAM-A','EM-U-SAM','EM-U-SAM-G','EM-U-P','EM-U-TP','EM-SAM-G3CP-GANC']
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for input,target in train_dataloader:
            ...
            with torch.set_grad_enabled(True):
                ...
                cur_loss = loss(pred,target) + (lossL1(seg[:,0:1,:,:],target)+lossL1(seg[:,1:2,:,:],target)+lossL1(seg[:,2:3,:,:],target))/3
                cur_loss.backward()
                # if k == 3:
                optimizer.step()
