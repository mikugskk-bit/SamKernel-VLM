...
from tqdm import *
import torchmetrics
from MydataLoader import MyDatasetK,PETCTDataset, MouseDataset
import argparse
...
def run():
    models = ['EM-U','EM-SAM','EM-SAMF','EM-SAM-A','EM-U-SAM','EM-U-SAM-G','EM-U-P','EM-U-TP','EM-U-SAM-G3CP','EM-U-SAM-G3CPD','EM-U-SAM-G3CPUF','EM-U-SAM-G3CP-Points','EM-U-SAM-G3CP-Boxs']
    ...
    test_dataset = MouseDataset(False)
    ...
    for input,target in test_dataloader:
        ...
        with torch.no_grad():
            ...
            # torch.unsqueeze
            # if k == 3:
            psnr = PSNR(pred,target.unsqueeze(1))
            ssim = SSIM(pred,target.unsqueeze(1))
    ...
    vepoch_ssim = total_ssim / len(test_dataloader) 
    print(vepoch_loss,vepoch_psnr,vepoch_ssim)
    np.savez('./mouseRes.npz',pred = preds, target = targets)
