...
for i in range(0,len(files),1):
    ...
    file3 = os.path.join(path3,files3[i])
    print(file)
    ...
    sam_mask3 = data3['sam_mask']
    point2= data3['point']
    box2 = data3['box']
    output3 = data3['output']
    ...
    for j in range(0,len(gt),1):
        ...
        psnr3 = PSNR(torch.Tensor(output3[j:j+1]),torch.Tensor(gt[j:j+1]),data_range=1.0)
        ...
        if index in indexs:
            si.append(i)
            sj.append(j)
            # plt.subplot(5,13,1+count*13)
            # if count == 0:
            #     plt.title('KEM')
            # plt.imshow(xo[j][2])
            # plt.axis('off')

            # ax = plt.subplot(5,13,2+count*13)
            # if count == 0:
            #     plt.title('prompt')
            ...
            for k in range(3):
                ax = plt.subplot(5,13,1+k*1+count*13)
                if count == 0:
            ...
            for k in range(3):
                ax = plt.subplot(5,13,5+k*1+count*13)
                if count == 0:
                    plt.title('prompt'+str(k),family='Times New Roman')
                plt.imshow(input[j,k])
            ...
            #all
            for k in range(3):
                ax = plt.subplot(5,13,9+k*1+count*13)
                if count == 0:
                    plt.title('prompt'+str(k),family='Times New Roman')
                plt.imshow(input[j,k])
                ...
                # plt.scatter(point[j][:k+1,0],point[j][:k+1,1], s=25, c='r')
                input_label = np.array([1,1,1])
                show_points(point2[j][:k+1], input_label[:k+1], plt.gca())
                plt.axis('off')
                # rect1 = patches.Rectangle((box[j][2+4*k], box[j][3+4*k]), box[j][0+4*k] - box[j][2+4*k], box[j][1+4*k]-box[j][3+4*k], 
                #                         linewidth=1, edgecolor='r',facecolor='none',angle=0)
                # ax.add_patch(rect1)
                show_box(box2[j][k*4:(k+1)*4],ax)
                show_mask(sam_mask3[j][k], plt.gca())

            plt.subplot(5,13,12+count*13)
            if count == 0:
                plt.title('output',family='Times New Roman')
            plt.imshow(output3[j][0],vmin=0,vmax=1)
            plt.text(0,zoom*2 - 1,'PSNR='+str('%.3f' % psnr3),fontsize=fsize, color = "r",family='Times New Roman')
            plt.axis('off')


            plt.subplot(5,13,13+count*13)
            if count == 0:
