...
def run():
    ...
    loss = nn.L1Loss()
    lossL1 = nn.HuberLoss()
    # lossp = lpips.LPIPS(net='alex').to(device)
    ...
    if model_name == models[0]:
        ...
        net = samG3(unfreeze=True).to(device)
    elif model_name == models[9]:
        net = samG3D().to(device)
    elif model_name == models[-2]:
        net = samG3(use_point=True,use_box=False).to(device)
    elif model_name == models[-1]:
        net = samG3(use_point=False,use_box=True).to(device)
    ...
    if model_name == models[7]:
        ...
        useCT = False
    train_dataset = PETCTDataset(is_train=True,use_ct=useCT)
    test_dataset = PETCTDataset(is_train=False,use_ct=useCT)
    # train_dataset = MyDatasetB(is_train=True)
    # test_dataset = MyDatasetB(is_train=False)

    train_dataloader = DataLoader(train_dataset,batch_size=opt.batch_size,shuffle=False,num_workers=0,pin_memory=True)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for input,target in train_dataloader:
            ...
            with torch.set_grad_enabled(True):
                ...
                pred,seg,seg2,p,b = net(input)

                # pred = pred.reshape(target.shape[0],target.shape[1],target.shape[2])
                # loss1 = lossL1(seg[:,0:1,:,:],torch.sigmoid(target))
                # loss2 = lossL1(seg[:,1:2,:,:],torch.sigmoid(target))
                # loss3 = lossL1(seg[:,2:3,:,:],torch.sigmoid(target))
                loss1 = lossL1(seg[:,0:1,:,:],torch.sigmoid(target))
                loss2 = lossL1(seg[:,1:2,:,:],torch.sigmoid(target))
                loss3 = lossL1(seg[:,2:3,:,:],torch.sigmoid(target))

                ploss1 = lossL1(seg2[:,0:1,:,:],torch.sigmoid(target))
                ploss2 = lossL1(seg2[:,1:2,:,:],torch.sigmoid(target))
                ploss3 = lossL1(seg2[:,2:3,:,:],torch.sigmoid(target))
                cur_loss = loss(pred,target) + (loss1+loss2+loss3)/3 + (ploss1+ploss2+ploss3)/3
                cur_loss.backward()
                # if k == 3:
                optimizer.step()
                ...
                ploss1s.append(ploss1.item())
                ploss2s.append(ploss2.item())
                ploss3s.append(ploss3.item())

                ps.append(p[0].detach().cpu().numpy())
                bs.append(b)
        ...
        np.savez('./lossRecord/'+str(epoch)+'.npz',loss1 = loss1s,loss2 = loss2s,loss3 = loss3s,ploss1 = ploss1s,ploss2 = ploss2s,ploss3 = ploss3s)
        # np.savez('./promptRecord/'+str(epoch)+'.npz',points = ps,boxs = bs)
