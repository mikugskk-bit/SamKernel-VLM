...
from tqdm import *
import torchmetrics
from MydataLoader import MyDatasetK,MyDatasetRawSino,MyDatasetRawSinoBrain
import argparse
...
from samt import samt,samG,samb,samba,samf,samG3

from samOsem import SamAgentOsem,SamAgentEm,SamAgentEmBrO,SamAgentEmNew,SamAgentOsemNew
...
def run():
    models = ['OSEM-SAM-8','EM-SAM-New','EM-SAM-New-i1','EM-SAM-New-Br-Old','EM-SAM-New-NU','OSEM-SAM-New-NU8']
    ...
    if model_name == models[0]:
        net = SamAgentOsem().to(device)
    elif model_name == models[1] or model_name == models[2]:
        net = SamAgentEm().to(device)
    elif model_name == models[3]:
        net = SamAgentEmBrO().to(device)
    elif model_name == models[4]:
        net = SamAgentEmNew().to(device)
    else:
        net = SamAgentOsemNew().to(device)
    ...
    for epoch in range(opt.epoch_count, opt.niter + opt.niter_decay + 1):
        ...
        for input,target in train_dataloader:
            ...
            with torch.set_grad_enabled(True):
                ...
                segLoss = 0
                for l in range(0,seg.shape[1]):
                    segLoss = segLoss + lossL1(seg[:,l:l+1,:,:],target)
                
                cur_loss = loss(pred,target) + segLoss/seg.shape[1]
                cur_loss.backward()
                # if k == 3:
                optimizer.step()
