...
batch_data = np.load('newDataset/osem-20e4-30-1-train.npz')['osem']
boxes = []

qbar = trange(len(batch_data)//3)
for i in range(len(batch_data)//3,len(batch_data)//3 * 2,1):
...
np.savez('./newDataset/osem-20e4-30-1-box-train-s2.npz',boxes=boxes)
