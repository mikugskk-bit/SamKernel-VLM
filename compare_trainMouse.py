...
from tqdm import *
import torchmetrics
from MydataLoader import MyDatasetK,PETCTDataset,MyDatasetRaw,MouseDatasetRaw
import argparse
...
def run():
    ...
    test_dataset = MouseDatasetRaw(False)
    ...
    for input,target in test_dataloader:
        ...
        with torch.no_grad():
            ...
            pred,diff_sino = net(target,target)
            ...
            ssim = SSIM(pred,target,data_range=1.0)
        ...
        qbar1.set_postfix(epoch = 0.0,loss=cur_loss.item())  # 进度条右边显示信息
    ...
    np.savez('./result/'+model_name+'-mouse.npz',pred=preds,target=targets,input=inputs)
