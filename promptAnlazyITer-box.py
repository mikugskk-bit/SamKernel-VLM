...
def show_box(box, ax,c):
    ...
    ax.add_patch(plt.Rectangle((x0, y0), w, h, edgecolor=colors[c], facecolor=(0,0,0,0), lw=2))
...
path = './promptResPoint-8E'

files = os.listdir(path)
count = 0

select = [46,50,75,77,82,30]
select = [13,40,53,54,57,83,95,129,140,157,172,183,258,357,368,371,376,378,391,410,498,499]
for i in range(510,800,1):
# for i in select:
    ...
    for j in range(box.shape[0]):
        ...
        plt.imshow(cv2.resize(gt[j,0],(256,256),interpolation=cv2.INTER_NEAREST),cmap='gray')
        plt.axis('off')

        # print(box[j],len(box[j])/4)
        for k in range(len(box[j])//4):
            show_box(box[j][k*4:k*4+4],plt.gca(),k)
        show_points(point[j], input_label, plt.gca())
        print(point[j],i)
    plt.show()
