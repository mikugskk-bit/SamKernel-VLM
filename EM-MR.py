...
def MLEMorKEM(sinogram,mul_factor,pet_noise,G,pet_gt,K=None,method_index='KEM',prefix='',radon=None,init=None,total_iter=None):
    ...
    for iter in range(total_iter):
        ...
        imgs.append(img)
        psnr2 = compare_psnr(pet_gt.numpy(),img,data_range=np.max(pet_gt.numpy()))
        ssim2 = compare_ssim(img,pet_gt.numpy(),data_range=np.max(pet_gt.numpy()))
        print(iter,psnr2,ssim2)
...
if __name__=="__main__":
    # 读取PET数据
    pet_data = np.load(r'F:\brainweb\pet_vol_slice_144by144_nt.npy')[10]
    mr_data = np.load(r'F:\brainweb\pet_vol_slice_144by144_nt_mr.npy')[10]
    # data1 = read_data("pet_new.mat")
    pet_gt,mr = np.array(pet_data),np.array(mr_data)
    pet_gt,mr = pad_and_center_image(np.stack([pet_gt,mr], axis=0),(128,128))
    print(pet_data.shape,mr_data.shape)
    ...
    radon2 = ParallelBeam(128,np.linspace(0,np.pi,128,False))
    # 使用EM生成PET图像
    ...
    kernel_size = 128*128
    sigma = 100 #0.05 100
    gaussian_matrix = gaussian_kernel(mr,kernel_size, sigma,J=3)

    # kernel = 1.0 * RBF(1.0)
    # gaussian_matrix = gaussian_matrix.reshape(-1,1)
    # print(gaussian_matrix)

    img = MLEMorKEM(sinogram,mul_factor,pet_noise,None,radon=radon2,pet_gt=pet_gt1,K=gaussian_matrix,method_index='KEM',prefix='KEM',total_iter=50)
    ...
    psnr1 = compare_psnr(pet_gt1.numpy(),img[0],data_range=np.max(pet_gt1.numpy()))
    ssim1 = compare_ssim(img[0],pet_gt1.numpy(),data_range=np.max(pet_gt1.numpy()))
    print(psnr1,ssim1)
    # 画图
    plt.figure()
    plt.subplot(1,5,1)
