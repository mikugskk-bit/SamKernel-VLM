...
def process_EM(args):
    ...
    gaussian_matrix = gaussian_kernel2(osem,mr,kernel_size, sigma,J=2)
    ...
    psnr1 = compare_psnr(pet_gt1,kemresult3,data_range=np.max(pet_gt1))
    ssim1 = compare_ssim(kemresult3,pet_gt1,data_range=np.max(pet_gt1))
...
if __name__=="__main__":
    ...
    all_data = np.load('/mnt/d/EM-reconstruction/result/SAM-decoder-Unf.npz')
    pet_data = all_data['target']
    SAM_data = all_data['pred']
    osem_data = all_data['input']
    ...
    for i in range(0,len(pet_data)):
        # plt.subplot(1,2,1)
        pet_gt,mr,osem = np.array(pet_data[i].reshape(128,128)),np.array(SAM_data[i].reshape(128,128)),np.array(osem_data[i].reshape(128,128))
        idata = np.stack([pet_gt,mr,osem])
    ...
    np.savez('./datasetB5N-KSAM.npz',kem=outputs)
